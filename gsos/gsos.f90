subroutine gsos(p,SS,la,al,TTh,Wm,TTar,maxit,thr,maxit2,thr2,niter,ipen,iact,dlz)
!
!  Arguments
!  =========
!
...
!
      ...
      double precision al,thr,thr2,dlz,rot1,rot2
...
10092 continue
      ...
      continue
10093 continue
      rot1=(-SS(j,j)-l1(j,j)+l2(j,j)*TTar(j,j)+SQRT((SS(j,j)+l1(j,j)-l2(j,j)*TTar(j,j))**2+4*l2(j,j)))/(2*l2(j,j))
      rot2=(-SS(j,j)-l1(j,j)+l2(j,j)*TTar(j,j)-SQRT((SS(j,j)+l1(j,j)-l2(j,j)*TTar(j,j))**2+4*l2(j,j)))/(2*l2(j,j))
      TTh(j,j)=max(rot1,rot2)
      Wm(j,j)=1.0/TTh(j,j)
10094 continue
      return
      end
subroutine gelnet_loop1(n,S,l3,l4,Th,W,Tar,maxit,thr,maxit2,thr2,outer,ipen,iact,dly)
      ...
      double precision shr, thr, thr2, throut, thrin, dly, dlx, test1, test2, help, delta, a, b, c, Thd
          ...
          if(ipen .eqv. .TRUE.) then
            if(Tar(j,j) .eq. 0.0) then
              if(l4(j,j) .eq. 0.0 ) then
                ...
                test1= (-help+SQRT((help)**2+4*l4(j,j)))/(2*l4(j,j))
                test2= (-help-SQRT((help)**2+4*l4(j,j)))/(2*l4(j,j))
                Th(j,j)= max(test1,test2)
